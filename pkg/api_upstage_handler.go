...
// HandleUpstageChat: Upstage API 스트리밍 처리를 수행한다.
func HandleUpstageChat(w http.ResponseWriter, r *http.Request, flusher http.Flusher, config AIConfig, provider, targetModel, sysPrompt, userPrompt string) {
	ctx := context.Background()

	apiKey, err := Decrypt(config.Upstage.APIKey)
	if err != nil {
		apiKey = ""
	}
	baseURL := "https://api.upstage.ai/v1"
	if targetModel == "" {
		targetModel = "solar-mini"
	}
